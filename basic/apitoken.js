...
function request(url, cookie, site) {
	...
	superagent
	.get(url)
	.set('User-Agent', 'Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/62.0.3202.94 Safari/537.36')
	.set('Accept','application/json,text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8')
	.set('Cookie', cookie)
	.ca(rootCas)
    .end(
        function(req,res){
			if (res && res.text) {
				...
				console.log('--- ' + site + ' --- ' + arr[2] + ' --- ' + new Date());
